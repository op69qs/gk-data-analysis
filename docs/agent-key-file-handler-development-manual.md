...
## 11. 本次验证结果（2026-07-27）

- 上报模块共 77 个单元/契约测试通过，1 个需本地样例路径的可选用例在普通回归中明确跳过；无失败。
- 另行指定项目根目录的脱敏样例后，样例回归通过：收入 4 个 Excel、支出 2 个 Excel、库存 3 个 Excel，均位于 ZIP 的业务子目录下；代码同时支持 ZIP 根目录直接放 Excel。
- 库存样例中含 `NNNNNNNNNN` / 纯 `N` 国库代码与名称时，与原 JAR 一致：原值入库、不拦截、不转换成市分库名称（Book9.xls 中 `stg.trs_tmis_stock` 已证实）。收入/支出仍保留国库名称含 `N` 的历史校验。库存具体日期原文已验证保留。
- 样例验证只读原 ZIP，解压和 spool 均在测试临时目录，没有连接或写入生产数据库，没有复制到 Git，没有上传到外部服务。
- 后端 `compile` 通过。整个 `org-tribe-system` 的全库 `mvn test` 会进入原 `SampleTest` 并尝试连接开发库 `100.71.11.54:25432`；当前环境不可达，因连接池持续重试而中止。该限制与上报模块的 77 个通过用例分开记录。
- 全部 reporting Vue/API 文件已用项目内 `vue-eslint-parser` 和 `babel-eslint` 做无规则语法校验，通过。项目未提供 ESLint 配置，`npm run lint` 会报配置/测试文件匹配错误。
- `npm run build` 仍被与本功能无关的 13 个历史可视化图片缺失阻断（`checkbox.png`、`cqIcon.png`、`default.png`、`firstIcon.png`、`secondIcon.png`、`thirdIcon.png`、`template1.png`～`template7.png`）；reporting 页面未出现编译错误。
- SQL 契约测已确认：七张新表和回滚齐全，菜单不创建调整页，人工 ADM 脚本不包含自动加工开关，`12` 仅含只读查询和 `EXPLAIN`。
