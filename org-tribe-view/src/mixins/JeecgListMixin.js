  ...
  methods: {
    ...
    handleImportExcel(info) {
      ...
      if (info.file.status === 'done') {
        ...
        this.$message.error(`文件上传失败: ${info.file.message} `)
      }
    },
    /* 图片预览 */
    getImgView(text) {
    ...
    /*停启用*/
    handleStopEnabled(record, param) {
      var that = this
      this.$confirm({
        title: '提示',
        content:
          !record.isleaf && record.state === '0'
            ? '禁用此项信息，同时也会禁用其所有子级，是否仍要继续该操作？'
            : '确认执行改操作?',
        onOk: function() {
          if (record.state === '1') {
            //判断父级是否禁用
            postAction(that.url.isDelBookOrg, param).then(res => {
              if (res.result === 'success') {
                postAction(that.url.stopEnabled, param).then(res => {
                  if (res.result === 'success') {
