...
export const taskPlanMixin = {
  ...
  methods: {
    ...
    /*下载模板*/
    downFileMethod() {
      location.href = this.url.downFile + '?X-Access-Token=' + JSON.parse(localStorage['pro__Access-Token']).value
    },
    ...
    //其他
    handleOther: function (params) {
      if (!this.url.other) {
        this.$message.error('请设置url.other属性!')
        return
      }
      var that = this
      postAction(that.url.other, params).then(res => {
        if (res.result === 'success') {
          that.$message.success(res.msg)
          that.loadData()
        } else {
