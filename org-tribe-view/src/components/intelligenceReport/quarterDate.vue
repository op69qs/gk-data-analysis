<template>
  <div>
    <el-input placeholder="请选择季" v-model="showValue" style="width:100%;" @focus="showSeason=true">
...
export default {
  ...
  created() {
    if (this.defaultValue) {
      console.log(this.defaultValue, 'ddddddvvvv')
  ...
  methods: {
    ...
    selectSeason(i) {
      ...
      // console.log(that.valueArr[i])
      // console.log(arr)
      let currentMonth = this.moment()
        .startOf('month')
        .format('YYYY-MM')
        .split('-')[1]
      ...
      } else if (that.year < currentYear) {
        that.getValue(that.year + arr[0] + '-' + that.year + arr[1])
        that.showSeason = false
        this.showValue = `${this.year}年${this.season}季度`
        this.$emit('value', `${this.year}Q${this.season}`)
      } else if (that.year == currentYear) {
      ...
      // console.log(that.year + arr[1])
      // that.showSeason = false
      // this.showValue = `${this.year}年${this.season}季度`
      // this.$emit('value', `${this.year}Q${this.season}`)
      // console.log(`${this.year}Q${this.season}`)
    }
  }
}
</script>
