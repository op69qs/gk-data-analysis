<template>
  <a-card :bordered="false">
    ...
    <a-alert class="tracking-tip" type="info" showIcon message="执行中的批次每 3 秒自动刷新；进入详情可查看每个阶段、文件与行错误。" />
...
export default {
  ...
  data() {
    return {
      ...
      pollTimer: null,
      rows: [],
      query: { sourceDomain: undefined, status: undefined, accountingPeriod: null, fileName: '' },
      ...
      statusOptions: [
        ...
        { value: 'SUCCEEDED', label: '成功' }, { value: 'PARTIALLY_SUCCEEDED', label: '部分完成' }, { value: 'FAILED', label: '失败' }
      ],
      columns: [
  ...
  methods: {
    ...
    reset() { this.query = { sourceDomain: undefined, status: undefined, accountingPeriod: null, fileName: '' }; this.search() },
