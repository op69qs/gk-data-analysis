<template>
  <div class="head">
    <span class="span" style="position: relative" @click="showDoubleMonth" ref="li"
      ><a-icon
        style="position: absolute; right: 10px; top: 12px; z-index: 100; color: rgba(0, 0, 0, 0.25); font-size: 14px"
        slot="suffixIcon"
        type="calendar" /><el-input v-model="choseQuarter" placeholder="请选择季度"></el-input
    ></span>
    <span v-if="timeType==2">~</span>
    <span v-if="timeType==1">~至今</span>
    <span v-if="timeType==2" class="span" style="position: relative" @click="showDoubleMonth1" ref="lis"
      ><a-icon
        style="position: absolute; right: 10px; top: 12px; z-index: 100; color: rgba(0, 0, 0, 0.25); font-size: 14px"
        slot="suffixIcon"
        type="calendar" /><el-input v-model="choseQuarter1" placeholder="请选择季度"></el-input
    ></span>
...
export default {
  props: ['choseQuarterData', 'choseQuarterData1','timeType'],
  data() {
    return {
      showTime1a: false,
      showTime1a1: false,
  ...
  mounted() {
    console.log('有物质'+this.choseQuarterData)
    this.choseQuarter = this.choseQuarterData
    this.choseQuarter1 = this.choseQuarterData1
  ...
  watch: {
    choseQuarterData(value) {
      console.log(value)
      this.choseQuarter = value
    },
    choseQuarterData1(value) {
      this.choseQuarter1 = value
    },
  },
  computed: {},
  methods: {
    ...
    selectQuarter(index) {
      ...
      if(this.choseQuarter1) {
        if(parseInt(this.choseQuarter.replace('-Q', '0')) > parseInt(this.choseQuarter1.replace('-Q', '0'))){
            this.$message.warning('您的开始时间大于开始时间请重新选择！')
        }
      }
