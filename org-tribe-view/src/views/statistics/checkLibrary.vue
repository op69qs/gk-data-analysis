<template>
  <a-card :bordered="false" class="card-area">
    ...
    <div id="first">
      ...
      <div class="table-page-search-wrapper zhibiao" :style="{height:height}">
        <!-- 搜索区域 -->
          ...
          <a-row :gutter="24">
            ...
            <a-col :md="9" :sm="9">
              <a-form-item label="指标类型" :labelCol="{span: 5}" :wrapperCol="{span: 18}" required>
                <a-select :disabled="disabled" v-model="queryParam.INDEX_TYPE" placeholder="请选择指标类型"
                          v-decorator="[ 'INDEX_TYPE', validatorRules.INDEX_TYPE]" allowClear>
                  <a-select-option :value="d.id" v-for="d in PLAN_TYPE_OPTIONS" :key="d.id">{{d.label}}
                  </a-select-option>
                </a-select>
              </a-form-item>
            </a-col>
            ...
            <a-col :md="9" :sm="9">
              <a-form-item
                ...
                required
              >
                <a-select :disabled="disabled" v-model="queryParam.INDEX_PERIOD" placeholder="请选择新增指标的周期"
                          v-decorator="[ 'INDEX_PERIOD', validatorRules.INDEX_PERIOD]" allowClear>
                  <a-select-option
                    :value="d.id"
        ...
        <a-row v-if="show1">
          ...
          <a-col :md="6" :sm="6" style="border: 1px solid #ccc;height:400px;">
            <a-row style="margin-bottom:20px;">
              <a-col :md="24" :sm="24" style="border-bottom:1px solid #ccc;">
                ...
                <a-input-search placeholder="请输入指标关键字查询" v-model="queryParam.filterText"
          ...
          <a-col :md="18" :sm="18" style="height:400px;border:1px solid #ccc;border-left:none;">
            ...
            <a-row>
              ...
              <a-col :md="8" :sm="8">
                <div class="rightDiv">
                  <div class="rightSpan" style="font-size:16px;">选择运算符：<span class="clear" @click="clear">清除</span>
                  </div>
                  <!-- <icon-font type="icon-jia" @click="jia" />
                  <icon-font type="icon-jian" @click="jian" />
                  <icon-font type="icon-chenghao" @click="cheng" />
        ...
        <a-row v-if="show2">
          <a-col :md="24" :sm="24" style="margin-bottom:15px;">查询SQL:<a style="margin-left:20px;" href="#">示例</a>
          </a-col>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        height: (document.body.clientHeight - 201 - 42) + 'px',
        content: ``,
        str: '',
        editorOption: {
          modules: {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            try {
              ...
              console.log(this)
              if (this.esql === '1' || this.$route.query.BUILD_TYPE === '1') {
                if (this.disableRun === false) {
                  ...
                  if (this.$route.query !== undefined && Object.keys(this.$route.query).length !== 0) {
                    updateMineIndex({
                      INDEX_NAME: this.queryParam.INDEX_NAME,
                      ...
                      INDEX_ID: this.$route.query.INDEX_ID,
                      ...
                      ADD_USERID: userId,
                      PARENT_ID:this.queryParam.PARENT_ID.value,
                    }).then(res => {
                      if (res.result == 'success') {
                        this.$message.success(res.msg)
                        //window.history.go(-1)
