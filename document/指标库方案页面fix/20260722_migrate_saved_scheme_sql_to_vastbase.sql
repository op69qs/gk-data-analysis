...
CREATE TABLE IF NOT EXISTS indicators_lib.lib_index_scheme_sql_backup_20260722 (
    id TEXT PRIMARY KEY,
    scheme_sql TEXT
);

CREATE TABLE IF NOT EXISTS visual_screen.vs_lib_index_scheme_sql_backup_20260722 (
    id TEXT PRIMARY KEY,
    scheme_sql TEXT
);

INSERT INTO indicators_lib.lib_index_scheme_sql_backup_20260722 (id, scheme_sql)
...
INSERT INTO visual_screen.vs_lib_index_scheme_sql_backup_20260722 (id, scheme_sql)
SELECT source.id, source.scheme_sql
...
BEGIN
    SELECT COUNT(*)
      ...
      FROM indicators_lib.lib_index_scheme source
      LEFT JOIN indicators_lib.lib_index_scheme_sql_backup_20260722 backup USING (id)
     WHERE backup.id IS NULL;
    IF missing_backup_count <> 0 THEN
        RAISE EXCEPTION '% indicator-library SQL rows are not backed up', missing_backup_count;
    END IF;

    SELECT COUNT(*)
      INTO missing_backup_count
      FROM visual_screen.vs_lib_index_scheme source
      LEFT JOIN visual_screen.vs_lib_index_scheme_sql_backup_20260722 backup USING (id)
     WHERE backup.id IS NULL;
    IF missing_backup_count <> 0 THEN
        RAISE EXCEPTION '% VIS SQL rows are not backed up', missing_backup_count;
    END IF;
END
$$;
...
BEGIN
    ...
    IF legacy_count <> 0 THEN
        RAISE EXCEPTION '% saved SQL rows still contain legacy MySQL syntax', legacy_count;
    END IF;
    ...
    LOOP
        ...
        EXECUTE 'SELECT * FROM (' || item.scheme_sql || ') migrated_scheme LIMIT 0';
        checked_count := checked_count + 1;
    END LOOP;

    RAISE NOTICE 'PASS: migrated and validated % saved SQL rows', checked_count;
END
$$;

COMMIT;

SELECT 'indicators_lib.lib_index_scheme' AS source_table, COUNT(*) AS migrated_rows
  FROM indicators_lib.lib_index_scheme
UNION ALL
SELECT 'visual_screen.vs_lib_index_scheme' AS source_table, COUNT(*) AS migrated_rows
  FROM visual_screen.vs_lib_index_scheme;
