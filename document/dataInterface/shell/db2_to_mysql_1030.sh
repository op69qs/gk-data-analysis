...
function load_to_mysql(){
	...
	do
		db2 "EXPORT TO $SHELL_HOME/data_db2/$table_list.txt OF DEL modified by nochardel codepage=1208 coldel| striplzeros SELECT * FROM $SCHEMA.$table_list  "
		...
		mysql -u$MS_USER -p$MS_PASSWD -h$HOST -e "DELETE FROM ods.$table_list ;
