...
@Slf4j
@RestController
@RequestMapping(value = "/alarmDetails", produces = MediaType.APPLICATION_JSON_VALUE)
public class AlarmDetailsController extends BaseController {
    ...
    //获取综合查询数据
    @PostMapping("/getAlarmDetailsData")
    public Map<String, Object> getAlarmDetailsData() {
        ...
        try {
            ...
            map.put("total", alarmDetailsService.getAlarmDetailsDataTotal(pageData));
            List<Map<String, Object>> dataList = alarmDetailsService.getAlarmDetailsData(pageData);
    ...
    public Object getTotalReportExcelOne(HttpServletRequest request, HttpServletResponse response) throws Exception {
        String newFileName = "monitorDetails_data";
        String title = "监测告警明细表";
        try {
            PageData pageData = this.getPageData();
            pageData.put("times", "BETWEEN '" + pageData.get("startTime") + "' AND '" + pageData.get("endTime") + "'");
            List dataList = alarmDetailsService.getAlarmDetailsData(pageData);
            //按日期设定当前Excel表格的名字
...
//            表头
            ...
            row1.createCell(5).setCellValue("告警次数");
            row1.createCell(6).setCellValue("告警内容");
            ...
            row1.getCell(6).setCellStyle(boderStyle);
            ...
            for (int i = 0; i < dataList.size(); i++) {
                ...
                row.createCell(5).setCellValue(db.get("alarm_value") != null ? db.get("alarm_value").toString() : "");
                row.createCell(6).setCellValue(db.get("description") != null ? db.get("description").toString() : "");
                ...
                row.getCell(6).setCellStyle(boderStyle);
            }
