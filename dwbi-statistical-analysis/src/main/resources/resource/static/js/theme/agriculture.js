...
//                  show: true
            ...
            data:[             
                {value:0, name:'农村贷款',itemStyle:{normal:{color:"#c12e34"}}},
                {value:0, name:'城市涉农贷款',itemStyle:{normal:{color:"#0098d9"}}},
...
//                  show: true
            ...
            data:[             
                {value:0, name:'农户贷款',itemStyle:{normal:{color:"#c12e34"}}},
                {value:0, name:'企业及各类组织涉农贷款',itemStyle:{normal:{color:"#e6b600"}}},
                {value:0, name:'非农户个人农林牧渔业贷款',itemStyle:{normal:{color:"#0098d9"}}},
...
//  	width:10,
    ...
    function getServiceData(params) {
        //地图的数据
        getservice('./analysis/agriculture/getAgricultureMap', params).done(function (res) {
            // console.log(res)
            var arr=[];
            console.log(res)
            if(res){
        ...
        //专项-涉农-涉农贷款按地域分析
        getservice('./analysis/agriculture/getAgricultureByArea', params).done(function (res) {
            if (res) {

                for (var i = 0; i < res.length; i++) {
                    pie1.series[0].data[i].value = res[i].value;
                    pie1.series[0].data[i].name = res[i].name;
                    pie1.legend.data.push(res[i].name)
        ...
        //专项-涉农-涉农贷款按主体分析
        getservice('./analysis/agriculture/getAgricultureByMain', params).done(function (res) {
            if (res) {

                for (var i = 0; i < res.length; i++) {
                    pie2.series[0].data[i].value = res[i].value;
                    pie2.series[0].data[i].name = res[i].name;
                    pie2.legend.data.push(res[i].name)
        ...
        //专项-涉农-涉农贷款按时序分析
        getservice('./analysis/agriculture/getAgricultureByDate',params).done(function (res) {
          if (res) {


            for (var i = 2; i < res.length; i++) {
                part2.series[i-2].data = res[i];
        ...
        //* 专项-涉农-涉农贷款按用途分析
        getservice('./analysis/agriculture/getAgricultureByUse', params).done(function (res) {
            if(res){
