...
public class CreateExcel_2 {
    ...
    public static boolean createExcel(
        ...
        try {
            ...
            excleAll(len_1,title_index,title,titlelist,zdlist,datalist,datalist,dataSize,wb,tableName);
            //判断是否存在目录. 不存在则创建
    ...
    private  static void excleAll(
                              ...
                              String[] zdlist,
                              List<Map<String,Object>> datalist,
                              List<Map<String,Object>> datalistFor,
                              int  dataSize,
                              HSSFWorkbook wb,
                              String tableName){
        ...
        if(dataSize<=60000) {
            ...
            for (int i = 0; i < len_1; i++) {
                Cell cellSub = row0.createCell(i);
                cellSub.setCellValue(zdlist[i]);
            ...
            for (int row = 0; row < datalist.size(); row++) {//数据行
                ...
                Map<String,Object> aa = datalist.get(row);
                Map<String,Object> data = JSONObject.parseObject(JSON.toJSONString(aa.get(tableName)));
            ...
            int c = datalist.size()%60000 == 0 ? (datalist.size()/60000) : (datalist.size()/60000)+1;
            for(int k=1;k<=c;k++){
//                System.out.println("开始第"+k+"个页签,当前页签从"+ ((k-1)*60000+1) +"到"+(k==c?datalist.size():k*60000));
                // 建立新的sheet对象（excel的表单）
                HSSFSheet sheet = wb.createSheet("第"+ ((k-1)*60000+1) +"条——第"+ (k==c?datalist.size():k*60000)+"条");
                // 在sheet里创建第一行，参数为行索引(excel的行)，可以是0～65535之间的任何一个
                HSSFRow row0 = sheet.createRow(0);
                for (int i = 0; i < len_1; i++) {
                    Cell cellSub = row0.createCell(i);
                ...
                //添加表中内容
                for (int row = (k-1)*60000; row < (k==c?datalist.size():k*60000) ; row++) {//数据行
                    //创建新行
    ...
    public static boolean createInspectSelfSumTable(
        ...
        try {
            ...
            styleMap.put("cellStyle", setDataStyle(cellStyle));
