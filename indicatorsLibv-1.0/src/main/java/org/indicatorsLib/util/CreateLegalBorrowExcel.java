...
public class CreateLegalBorrowExcel {
    /**
     * 生成Excel 并放到指定位置
    ...
    public static boolean createExcel(String title,
        ...
        try {
            ...
            cel1.setCellValue("被检查对象：" + datalist.get(0).get("CHECKED_ORG_DSCR").toString());

            HSSFRow row2 = sheet.createRow(2);
            Cell cel2=row2.createCell(0);
            cel2.setCellValue("检查项目：" + datalist.get(0).get("INSPECT_ITEM_DSCR").toString());
            ...
            Cell lastCel  =lastRow.createCell(0);
            lastCel.setCellValue("备注：本执法检查调阅资料清单一式两份，检查组一份，被检查对象一份。");
