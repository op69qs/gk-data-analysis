...
with expected_object(schema_name, object_name, object_kind, jar_operation, business_purpose) as (
    values
        ...
        ('agent_key_file', 'tims_file_income', 'TABLE', 'INACTIVE_NO_WRITE', 'JAR非定时中间路径；本期不写'),
        ('agent_key_file', 'tims_file_payout', 'TABLE', 'INACTIVE_NO_WRITE', 'JAR非定时中间路径；本期不写'),
        ('agent_key_file', 'tims_file_stock', 'TABLE', 'INACTIVE_NO_WRITE', 'JAR非定时中间路径；本期不写'),
        ...
        ('edw', 'income_report_detail_stat', 'TABLE_OR_VIEW', 'INACTIVE_NO_API', 'JAR调整页基线；本期不暴露'),
        ('edw', 'payout_report_detail_stat', 'TABLE_OR_VIEW', 'INACTIVE_NO_API', 'JAR调整页基线；本期不暴露'),
        ('edw', 'reprot_update_record', 'TABLE', 'INACTIVE_NO_WRITE', 'JAR人工调整记录；本期不写'),
