...
public class SaticScheduleTask {
    ...
    @Autowired
    private TreasuryAccessHouseService treasuryAccessHouseService;
...
//    @Scheduled(cron = "0/10 * * * * ?")
    private String accessList(){
        String alarmUrl = "http://11.8.165.25:8000/FDSP/services/getToken?loginName="+loginName+"&loginPassword="+loginPassword;
...
//        obj.put("paramList", JSONArray.parseArray(dataBody));
        ...
        if (responseEntity.getStatusCodeValue() == 200) {
            ...
            if (record != null) {
                log.info("获取三证合一总条数: " + record.get("total").toString());
...
//                    ResponseEntity<String> responseEntitys = restTemplate.exchange(url, HttpMethod.GET, entity, String.class);
                    ...
                    if (responseEntitys.getStatusCodeValue() == 200) {
                        ...
                        if(records != null && records.get("list") != null){
                            ...
                            if (list.size() > 0) {
                                list.forEach(map -> {
//                                    log.info("获取三证合一总条数: " + map);
                                    alarmList.add(map);
                                });
                                count += alarmList.size();
                                log.info("三证合一信息入库"+alarmList.size()+"条：[" + url + "]"+"---token="+token+"===dataDte1="+dataDte1);
...
//    @Scheduled(cron = "0 20 20 * * ?") // 每天凌晨7点执行一次
...
//        ca1.add(Calendar.DATE, -10);
        ...
        // header填充
//        String dataBody = "[{'name':'INFODATE','match':'gte','value':'"+dataDte1+"'},{'name':'INFODATE','match':'lte','value':'"+dataDte2+"'}]";
        String dataBody = "[{'name':'DATAUPTIME','match':'gte','value':'"+dataDte1+"'}]";
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.valueOf(MediaType.APPLICATION_JSON_VALUE));
        headers.set("Authorization",token);
        //放入body中的json参数
...
//            log.info(record.toString());
            if (record != null) {
                log.info("获取企业信息总条数: " + record.get("total").toString());
//                treasuryAccessService.delEnterpriseBasicInformation();
                int sizeFor=0;
                for(int i=page;i<=(Integer.parseInt(record.get("total").toString())/httpPage+1);i++){
                    if(i >= size){
                        log.info("获取企业信息结束从"+i+"页---到---" + size + "页（不包含"+size+"）");
                        return;
                    }
                    List<Map<String, Object>> alarmList = new ArrayList<>();
...
//                        log.info(records.toString());
                        if(records != null && records.get("list") != null){
                            ...
                            if (list.size() > 0) {
                                ...
                                log.info("获取企业信息"+alarmList.size()+"条：[" + url + "]"+"---token="+token);
                            }else{
                ...
                log.info("获取企业信息取数成功：[" + count + "]条");
                log.info("获取企业信息结束从"+page+"页---到---" + sizeFor + "页");
            }
        } else {
...
//        String alarmUrl = "http://11.8.165.25:8000/FDSP/services/getData/ff8080817490ac9a0174afb669360151/1/10000";
        ...
        if (responseEntity.getStatusCodeValue() == 200) {
            ...
            Map<String, Object> record = (Map<String, Object>) jsonObject.get("data");
            if (record.size() > 0) {
                log.info("获取非企业团体基本信息总条数: " + record.get("total").toString());
...
//                            treasuryAccessHouseService.insertNonEnterpriseBasicInformationHouse(alarmList);
                ...
                log.info("非企业团体基本信息取数成功：[" + count + "]条");
            }
        } else {
