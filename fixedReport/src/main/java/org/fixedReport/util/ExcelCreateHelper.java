...
public class ExcelCreateHelper {
    ...
    public static boolean createSimpleExcel(
        ...
        try {
            ...
            int rowFooterIndex1 = datalist.size() + 4;
            HSSFRow rowFooter1 = sheet.createRow(rowFooterIndex1);
            if (footer_1 != null) {
                Cell footer1 = rowFooter1.createCell(0);
                footer1.setCellValue(footer_1);
                if (divlen >= 2) {
                    sheet.addMergedRegion(new CellRangeAddress(rowFooterIndex1, rowFooterIndex1, 0, 1));
                }
            }
            if (footer_2 != null) {
                Cell footer2 = rowFooter1.createCell(len_1 - 1);
                footer2.setCellValue(footer_2);
                if (divlen >= 2) {
                    sheet.addMergedRegion(new CellRangeAddress(rowFooterIndex1, rowFooterIndex1, 0 + divlen, 0 + divlen + 1));
                }
            }
            int rowFooterIndex2 = datalist.size() + 5;
            HSSFRow rowFooter2= sheet.createRow(rowFooterIndex2);
            if (footer_3 != null) {
                Cell footer3 = rowFooter2.createCell(0);
                footer3.setCellValue(footer_3);
            }
