...
public class CreateExcel_2 {
    ...
    public static boolean createInspectStatisticsTable(
        ...
        try {
            ...
            excleAll(map,cellIndex,len_1,title_index,title, filepath, filename, titlelist, zdlist,  datalist,datalist, ifAll, dataSize,wb);
            //判断是否存在目录. 不存在则创建
...
private  static void excleAll(Map map,
        int cellIndex,
                              int len_1,
                              int title_index,
                              String title,
    ...
    if(dataSize<=60000) {
        ...
        for(int k=1;k<=c;k++){
            ...
            for (int row = (k-1)*60000; row < (k==c?datalist.size():k*60000) ; row++) {//数据行
                ...
                //获取该行的数据
                @SuppressWarnings("unchecked")
                Map<String, Object> data = datalistFor.get(row);
                for (int col = 0; col < zdlist.length; col++) {//列
                    //数据从第一列开始
                    //创建单元格并放入数据
                    Cell dataCell = newrow.createCell(col);
